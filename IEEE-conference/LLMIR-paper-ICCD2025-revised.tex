...
\caption{Verified MVP evaluation (Nature-style panels). \textbf{a,} MVP-A block-size rewrite. \textbf{b,} MVP-B normalized latency with prefix reuse. \textbf{c,} MVP-C relative decode throughput (GPU path vs NumPy reference). Reproduce via \texttt{docs/PAPER\_REVISION\_TRACEABILITY.md}.}
\label{fig:mvp_eval}
\end{figure}
...
\caption{Block-size sweep (Nature style; MVP-A aligned). \textbf{a,} Throughput vs block size on LLaMA-2-13B (ShareGPT-shaped workload, A100-80GB). \textbf{b,} Fragmentation metric $F(B)$ from Algorithm~\ref{alg:block_opt}. MVP-A verified rewrite: oversized attributes (e.g., 1024) reduce to 32 for short sequences in CI.}
...
\caption{Attention optimization speedups (Nature style, log-scaled sequence length). Sliding window reaches 2.15$\times$ at 4096 tokens; Flash Attention yields 1.5--1.7$\times$. Micro-benchmark: A100-80GB, head dim 128, batch 8.}
...
\textbf{Measurement gap}: Table~\ref{tab:multi_model} and several micro-benchmark figures use illustrative targets until the multi-model GPU harness feeds \texttt{llmir-benchmark} JSON into \texttt{scripts/plot\_from\_results.py}. Verified claims are scoped to MVP-A/B/C (Fig.~\ref{fig:mvp_eval}).
...
Our open-source stack verifies the compiler-to-serving path through MVP-A/B/C, while projected multi-model results indicate 22.4\% over vLLM and 37.8\% over SGLang at scale. LLMIR's compile-time optimization with minimal runtime overhead is a promising direction for LLM inference systems; figures use Nature-style visualization and link to reproducible harnesses in the repository.
