...
\textbf{Dynamic Sequence Lengths}: While LLMIR handles dynamic sequences through bounded analysis and fallback paths, extremely variable workloads may not benefit fully from compile-time optimization. Future work will explore adaptive compilation with runtime feedback.

\textbf{Architecture Coverage}: Current implementation focuses on transformer-based architectures. Extending to state-space models (Mamba) and linear attention variants requires additional dialect operations, which we plan to address in future releases.

\textbf{Distributed Training}: LLMIR currently targets inference. Extending compilation support to training with gradient checkpointing and optimizer state management would provide end-to-end optimization.
...
Our evaluation across diverse models (LLaMA, Phi, Qwen, DeepSeek) demonstrates consistent improvements: 22.4\% over vLLM, 37.8\% over SGLang, and competitive performance with TensorRT-LLM while maintaining model quality (perplexity within 0.15\%). LLMIR's approach of compile-time optimization with minimal runtime overhead represents a promising direction for LLM inference systems. The complete implementation is available as open-source software.
