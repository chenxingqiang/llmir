...
\hline
Qwen2.5-7B (meas.\textsuperscript{‡}) & --- & 94 & --- & --- & --- \\
\hline
...
\end{center}
\textsuperscript{$\dagger$}\footnotesize Projected rows (A100-80GB, batch 8). \textsuperscript{‡}Measured: gpt2 \texttt{llmir-paged} 27 tok/s vs HF 70 tok/s (CPU, batch 1, 64 prompt tok) from \texttt{paper\_results.json}; Qwen2.5-7B vLLM 93.7 tok/s (A800, batch 1) from \texttt{a800\_qwen\_inference.json}. LLMIR \texttt{llmir-paged} on Qwen pending GPU CI.
\end{table}

\begin{table}[htbp]
\caption{Measured Harness: HF vs LLMIR-paged vs vLLM (same prompt length)}
\begin{center}
\small
...
\hline
gpt2 & 70 & 27 & --- & \texttt{paper\_results.json} \\
\hline
Qwen2.5-7B & 46 & --- & 94 & A800 batch=1 \\
\hline
\end{tabular}
\label{tab:measured_harness}
\end{center}
\footnotesize Tokens/sec; gpt2 on CPU (64 prompt, 8 gen). Qwen on A800-80GB (Feb 2026). vLLM skipped on CPU harness.
\end{table}
...
Key observations: (1) \textsuperscript{‡}Measured rows (Table~\ref{tab:measured_harness}) show HF can beat \texttt{llmir-paged} on short CPU prompts; LLMIR value is compile-time optimization plus prefix reuse (Fig.~\ref{fig:prefix_ttft}). (2) Projected rows suggest LLMIR targets gains over runtime systems when GPU harness completes. (3) Qwen2.5-7B vLLM throughput (94 tok/s, A800 batch=1) anchors one real baseline column.
