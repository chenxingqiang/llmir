...
\end{center}
\textsuperscript{$\dagger$}\footnotesize Projected single-GPU targets (A100-80GB, batch 8); populate from \texttt{llmir-benchmark --compare} JSON. Heatmap: Fig.~\ref{fig:multi_model}.
\end{table}
...
Figure~\ref{fig:prefix_cache} shows MVP-B behaviour: after \texttt{warm\_prefix}, subsequent requests skip repeated prefill for the shared system prompt. KV-layer simulation isolates prefix reuse; \texttt{llmir\_paged} end-to-end gains depend on prompt length and overlap.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{figures/paper-only/prefix_cache_nature.pdf}
\caption{ShareGPT-shaped prefix evaluation (Nature style). \textbf{a,} Per-request normalized latency with/without warm prefix. \textbf{b,} Speedup summary: KV simulation vs \texttt{llmir\_paged} E2E (gpt2-class models in CI).}
