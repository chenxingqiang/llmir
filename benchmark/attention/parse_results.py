...
def generate_visualizations():
    ...
    if flash_results and mqa_results and pruned_results:
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(plots_dir, 'speedup_vs_seq_length.png'), dpi=300, bbox_inches='tight')
        plt.close()
        ...
        if mqa_results:
            ...
            plt.grid(True)
            plt.savefig(os.path.join(plots_dir, 'mqa_memory_reduction.png'), dpi=300, bbox_inches='tight')
            plt.close()
    ...
    if threshold_results:
        ...
        plt.grid(True)
        plt.savefig(os.path.join(plots_dir, 'threshold_effect.png'), dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.grid(True)
        plt.savefig(os.path.join(plots_dir, 'threshold_accuracy.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    if topk_results:
        ...
        plt.grid(True)
        plt.savefig(os.path.join(plots_dir, 'topk_effect.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    if 512 in flash_results and 512 in mqa_results and 512 in pruned_results:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(plots_dir, 'timing_comparison_512.png'), dpi=300, bbox_inches='tight')
        plt.close()
