...
for seq_len in "${SEQ_LENGTHS[@]}"; do
    ...
    ./simple_attention_benchmark --seq $seq_len --context $seq_len --trials 10 > results/flash_attn_seq${seq_len}.txt
done
...
for seq_len in "${SEQ_LENGTHS[@]}"; do
    echo "Testing sequence length: $seq_len"
    ./simple_mqa_benchmark --seq $seq_len --context $seq_len --trials 10 > results/mqa_seq${seq_len}.txt
done
...
for seq_len in "${SEQ_LENGTHS[@]}"; do
    echo "Testing sequence length: $seq_len"
    ./simple_pruned_attention_benchmark --seq $seq_len --context $seq_len --trials 10 > results/pruned_attn_seq${seq_len}.txt
done
...
for threshold in "${THRESHOLDS[@]}"; do
    ...
    ./simple_pruned_attention_benchmark --seq 512 --context 512 --threshold $threshold --trials 10 > results/pruned_attn_threshold${threshold}.txt
done
...
for topk in "${TOPK_VALUES[@]}"; do
    ...
    ./simple_pruned_attention_benchmark --seq 512 --context 512 --topk $topk --trials 10 > results/pruned_attn_topk${topk}.txt
done
