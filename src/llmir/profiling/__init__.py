...
class Profiler:
    ...
    def trace(self, name: str, event_type: EventType = EventType.CUSTOM, **metadata):
    ...
    def record_event(
        self, name: str, event_type: EventType, duration_ms: float, **metadata
    ):
...
class ProfileReport:
    ...
    def __init__(
        self, events: List[ProfileEvent], counters: Dict[str, int], total_time_ms: float
    ):
        self.events = events
    ...
    def print_summary(self):
        ...
        for stat in sorted_stats:
            print(
                f"{stat.name:<25} {stat.count:>8} {stat.total_ms:>9.2f}ms "
...
class MemoryProfiler:
    ...
    def get_peak_memory(self) -> int:
        ...
        if not self._snapshots:
            return 0
        return int(max(s["allocated"] for s in self._snapshots))
...
def profile(name: Optional[str] = None, event_type: EventType = EventType.CUSTOM):
