...
class PatternMatcher:
    ...
    def _extract_manual_attention_pattern(
        self, softmax_node, graph
    ) -> Optional[AttentionPattern]:
...
class MLIRBuilder:
    ...
    def build_module(
        ...
        for attention_pattern in attention_patterns:
            lines.append(self._build_attention_op(attention_pattern))

        for linear_pattern in linear_patterns:
            if linear_pattern.is_quantized:
                lines.append(self._build_quantized_matmul(linear_pattern))
            else:
                lines.append(self._build_linear_op(linear_pattern))
    ...
    def _build_function_signature(self, model_config: Optional[Dict[str, Any]]) -> str:
        ...
        if self.config.enable_kv_cache:
            ...
            num_heads = (
                model_config.get("num_attention_heads", 32) if model_config else 32
            )
            ...
            args.append(
                f"    %kv_cache: !llm.paged_kv_cache<{dtype}, {num_layers}, {num_heads}, {head_dim}, {block_size}, {max_seq}>"
            )
...
class PyTorchImporter:
    ...
    def import_model(
        ...
        mlir_text = builder.build_module(
            attention_patterns, linear_patterns, self._model_config
        )
    ...
    def _extract_model_config(self, model) -> Dict[str, Any]:
        ...
        if hasattr(model, "config"):
            ...
            config["num_attention_heads"] = getattr(
                hf_config, "num_attention_heads", 32
            )
            config["num_key_value_heads"] = getattr(
                hf_config, "num_key_value_heads", config["num_attention_heads"]
            )
