...
class PagedKVCache:
    ...
    def append(
        ...
        return cast(np.ndarray, block_indices)
    ...
    def lookup(
        ...
        for batch_idx in range(batch_size):
            ...
            if seq_len_i <= 0:
                continue
            seq_id = (
                int(block_indices[batch_idx, 0])
                if batch_idx < block_indices.shape[0]
                else batch_idx
            )
    ...
    def get_sequence_length(self, seq_id: int) -> int:
        ...
        if seq_id in self._sequences:
            return int(self._sequences[seq_id]["length"])
        return 0
...
class DistributedKVCache:
    ...
    def __init__(
        self, config: KVCacheConfig, sharding_config: Optional[ShardingConfig] = None
    ):
        self.config = config
    ...
    def _initialize(self):
        ...
        for _device_id in self.sharding_config.device_ids:
...
class SpeculativeKVCache:
    ...
    def append_speculative(
        self, keys: np.ndarray, values: np.ndarray, seq_id: int, branch_id: int = 0
    ):
    ...
    def verify(
        self, seq_id: int, branch_id: int, target_logprobs: np.ndarray
    ) -> "VerificationResult":
        """Verify speculative tokens."""
        # Simplified verification - in practice would compare logprobs
        return VerificationResult(
            accepted_count=0, rejected_position=-1, acceptance_rate=0.0
        )
