...
class TestModelConfig:
    ...
    def test_is_gqa(self):
        """Test GQA detection."""
        gqa_config = ModelConfig(num_attention_heads=32, num_key_value_heads=8)
        mha_config = ModelConfig(num_attention_heads=32, num_key_value_heads=32)
    ...
    def test_queries_per_kv(self):
        ...
        config = ModelConfig(num_attention_heads=32, num_key_value_heads=8)
...
class TestModelRegistry:
    ...
    def test_register_custom(self):
        ...
        custom_config = ModelConfig(
            architecture=ModelArchitecture.CUSTOM, num_layers=24, hidden_size=2048
        )
...
class TestModelMemoryEstimator:
    ...
    def test_kv_cache_memory(self):
        ...
        config = ModelConfig(
            num_layers=32, num_attention_heads=32, num_key_value_heads=32, head_dim=128
        )
        estimator = ModelMemoryEstimator(config)

        memory = estimator.estimate_kv_cache_memory(batch_size=1, seq_len=2048)
    ...
    def test_find_max_batch_size(self):
        ...
        max_batch = estimator.find_max_batch_size(
            memory_budget=80 * 1024**3, seq_len=2048
        )
    ...
    def test_total_memory(self):
        ...
        total = estimator.estimate_total_memory(batch_size=8, seq_len=2048)
